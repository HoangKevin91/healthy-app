...
type Props = {
    listCategory: FoodMenu[]
}

export default function ListCategory(props: Props) {
    return (
        <div className="flex mx-auto my-[25px] container">
            {props.listCategory.map((c, y) => {
                return <div key={y} className="w-1/4">
                    <div className="flex items-center justify-center flex-col bg-no-repeat mx-auto bg-foodMenu-pattern bg-cover w-[134px] pt-[33px] pb-[33px]">
                        ...
                        <span className="text-xl text-white mt-1">{c.title}</span>
                    </div>
                </div>
            })}
        </div>
